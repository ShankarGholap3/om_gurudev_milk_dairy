  ...
  m_statsButton(nullptr)
{
  ...
  this->setWindowIcon(QIcon(":images/FlipBagIcon.png"));
...
{
   this->connect(ui->actionSelect_File_To_Flip, &QAction::triggered,
...
{
  m_flipButton = new QPushButton(nullptr);


  QPixmap pixmapFlip(":images/FlipBagIcon2.png");
  QIcon buttonIconFlip(pixmapFlip);
  m_flipButton->setToolTip("Pereform Flip");
  m_flipButton->setIcon(buttonIconFlip);
  ui->mainToolBar->addWidget(m_flipButton);
  ...
  this->connect(m_flipButton, &QPushButton::clicked,
                this, &MainWindow::onFlipdDtaFile);
...
void MainWindow::generateFipName()
{
  ...
  m_flipFile = nameStub + g_flipSuffix + tail;
  ui->lineEditOutputFile->setText(m_flipFile);
}
...
//------------------------------------------------------------------------------
void MainWindow::onFlipdDtaFile()
{
  m_flipFile = ui->lineEditOutputFile->text();
  if (m_currentFile.isEmpty() ||
      m_flipFile.isEmpty())
  {
    QMessageBox::warning(this,
  ...
  m_console->logString("Beginning data flip!");
  std::string inFile   = m_currentFile.toLocal8Bit().data();
  std::string flipFile = m_flipFile.toLocal8Bit().data();
  /*

 * @brief flipVolumeFileDataInY
 * @param inFile  - Read input file
 * @param outFile - Written output file
 * @param clipVal - Clip Value
 * @return

bool flipVolumeFileDataInY(std::string& inFile,
                    std::string& outFile,
                    bool setLowerCorner,
                    int16_t clipVal = CHECKPOINT_SIGNAL_CLIP_VALUE);

   */

  bool flipResult  =  analogic::workstation::flipVolumeFileDataInY(inFile,    // OK
                                                                   flipFile,  // OK
                                                                   true,      // OK
                                                                   0xFFF);    // OK

  if (flipResult) m_console->logString("Data flip succesfull.");
  else m_console->logString("Data flip faild!");
}
